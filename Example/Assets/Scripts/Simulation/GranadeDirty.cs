...
public class GranadeDirty : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isClean == true)
        {
            ...
            {
                //ObjectyManager.Instance.ObjectyPools["ObjectyPool"].Spawn("CleanGrenade", transform.position, new Vector3(90, 0, 0)).transform.SetParent(transform.parent); // When we clear the weapon, it spawns a new weapon onto the platform.
    ...
    { /*
        if (isKnifePlaced == false)
        {
            ...
            spawnedWeapon = ObjectyManager.Instance.ObjectyPools["ObjectyPool"].Spawn("CleanGrenade", gameManager.weaponPositions[gameManager.currentWeaponIndex].position);
            spawnedWeapon.transform.SetParent(gameManager.targetPosition.parent);
            spawnedWeapon.transform.eulerAngles = new Vector3(0, 0, 0);
            spawnedWeapon.transform.localScale = new Vector3(1.5f, 1.5f, 1.5f);
            spawnedWeapon.GetComponent<GrenadeRunner>().enabled = true;
