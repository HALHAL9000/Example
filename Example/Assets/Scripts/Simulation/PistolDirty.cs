...
public class PistolDirty : MonoBehaviour
{
   ...
   // [SerializeField] private List<P3dChangeCounter> counters;
    ...
    private void Update()
    {
        if (isClean == true)
        {
            ...
            {
                //ObjectyManager.Instance.ObjectyPools["ObjectyPool"].Spawn("CleanPistol", transform.position, new Vector3(180, 0, -90)).transform.SetParent(transform.parent); // When we clear the weapon, it spawns a new weapon onto the platform.
    ...
    void PlaceWeapon()
    {
        ...
        { /*
            ...
            spawnedWeapon = ObjectyManager.Instance.ObjectyPools["ObjectyPool"].Spawn("CleanPistol", gameManager.weaponPositions[gameManager.currentWeaponIndex].position);
            ...
            spawnedWeapon.GetComponent<PistolRunner>().enabled = true;
